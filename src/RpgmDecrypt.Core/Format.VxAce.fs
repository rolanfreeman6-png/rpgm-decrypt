...
/// RPG Maker VX Ace archive walker — extension `.rgss3a`, version byte
/// 0x03. The on-the-wire byte layout differs from XP/VX:
///   size, name_len, name, offset  (size BEFORE name)
/// Reference: same docs as XP/VX; check against uuksu public source.
[<RequireQualifiedAccess>]
module VxAce =

    open System

    // "RGSSAD\0" — 7 bytes, hard-coded.
    let magicKey : byte[] =
    ...
    /// Layout for VXAce (per reverse-engineered public spec):
    ///   size (u32 LE), name_len (u32 LE), name (name_len bytes),
    ///   offset (u32 LE).
    let parse (buf: byte[]) : Result<Entry list * int, ParseError> =
        if buf.Length < 8 then Error ShortHeader
            ...
            while keepGoing do
                if pos + 8 > buf.Length then keepGoing <- false
                else

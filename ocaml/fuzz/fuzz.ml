...
let mv_seed =
  Crypto.xor_transform key16
    (Bytes.of_string "\x89PNG\r\n\x1a\n\x00\x00\x00\x0dIHDR")

let mz_seed =
  let z = Zip.open_out tmp_pak in
  Zip.add_entry
    (Bytes.to_string (Crypto.xor_transform key16 (Bytes.make 16 'A')))
    z "www/img/a.png";
  Zip.close_out z;
...
let vxace_seed =
  ...
  Bytes.set b 7 '\x03';
  for i = 12 to 91 do
    Bytes.set b i (Char.chr (i * 13 land 0xFF))
  done;
  b

let seeds =
  [| build_rgssad 0x01; build_rgssad 0x02; vxace_seed; mv_seed; mz_seed |]

let mutate rnd =
  if Random.State.int rnd 10 < 3 then
    Bytes.init (Random.State.int rnd 1024) (fun _ ->
        Char.chr (Random.State.int rnd 256))
  else begin
    ...
    | 0 ->
        for _ = 1 to 1 + Random.State.int rnd 30 do
          if Bytes.length b > 0 then
            Bytes.set b
              (Random.State.int rnd (Bytes.length b))
              (Char.chr (Random.State.int rnd 256))
        done;
        b
    | 1 ->
        if Bytes.length b = 0 then b
        else Bytes.sub b 0 (Random.State.int rnd (Bytes.length b))
    | _ ->

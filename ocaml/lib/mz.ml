(* Port of Format.Mz.fs — MZ `.pak` extraction. The `.pak` is a plain ZIP;
...
let open_pak (path : string) : (Zip.in_file, open_error) result =
  try
    ...
    let head =
      if Bytes.length b >= 4 then Bytes.sub b 0 4 else Bytes.sub b 0 (Bytes.length b)
    in
    ...
    let results =
      ...
      |> List.filter_map (fun (e : Zip.entry) ->
             if e.Zip.is_directory then None
               ...
               Some { entry_name = e.Zip.filename; plaintext_kind = kind; bytes = plain }
             end)
    in
    Ok results

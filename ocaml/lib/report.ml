(* Port of Report.fs — orchestrator: walk -> classify -> decrypt -> write a
...
let rename_by_kind (rel_path : string) (kind : string) : string =
  ...
  | "webp" ->
      ...
      let ext = "." ^ kind in
      if dir = "" || dir = "." then stem ^ ext else Filename.concat dir (stem ^ ext)
  | _ -> rel_path
...
let normalize (path : string) : string =
  ...
  let rec go acc = function
    ...
    | ".." :: rest -> ( match acc with _ :: tl -> go tl rest | [] -> go acc rest)
...
let safe_join (out_dir : string) (rel : string) : string option =
  ...
  | Some full ->
      ...
      if full = root || (String.length full >= rl && String.sub full 0 rl = root_sep)
      then Some full
      else None
          ...
          try
            ...
            | None ->
                ...
                let why =
                  Printf.sprintf "unsafe entry path blocked (traversal): %s"
                    e.Vxace.name
                in
                summary := Types.tally (Types.Failed (e.Vxace.name, why)) !summary;
            ...
            | Some output_path ->
                ...
                summary :=
                  Types.tally
                    (Types.Decrypted
                       (output_rel, Int64.of_int (Bytes.length plain), Types.VXAce))
                    !summary;
          ...
          with ex ->
            ...
            summary :=
              Types.tally
                ...
                !summary;
            cfg.on_event
              (Log.Failed (d_abs, Printf.sprintf "decode error on %s" e.Vxace.name)))
        entries;
      if not !failed_here then begin
        let manifest_path = Filename.concat cfg.out_dir (d_rel ^ ".entries.txt") in
        ...
        summary := Types.tally (Types.PassedThrough (manifest_path, Types.VXAce)) !summary;
...
let write_rgssad_manifest (cfg : config) (d : Types.detected_file)
    (entries : Rgssad_core.entry list) (fmt : Types.format) (summary : Types.run_summary ref) : unit =
  let manifest_path = Filename.concat cfg.out_dir (d.Types.rel_path ^ ".entries.txt") in
  (if not cfg.dry_run then
     let txt =
       entries
  ...
  cfg.on_event (Log.Decrypt (d.Types.abs_path, manifest_path, Types.format_to_string fmt))
...
let run (cfg : config) : Types.run_summary =
  ...
  List.iter
    (fun (d : Types.detected_file) ->
      ...
      cfg.on_event (Log.Detected (d.Types.abs_path, Types.format_to_string d.Types.format));
      ...
      | Types.MV -> (
          ...
          | Ok (bytes, kind, was) ->
              if was then begin
                ...
                summary :=
                  Types.tally
                    (Types.Decrypted
                       (real_kind_out, Int64.of_int (Bytes.length bytes), Types.MV))
                    !summary;
                cfg.on_event (Log.Decrypt (d.Types.abs_path, real_out_abs, "MV"))
              end
              else begin
                if not cfg.dry_run then copy_through d.Types.abs_path out_abs;
                summary := Types.tally (Types.PassedThrough (out_rel, Types.MV)) !summary;
          ...
          | Error msg ->
              ...
              summary := Types.tally (Types.Failed (d.Types.rel_path, msg)) !summary;
      ...
      | Types.MZ -> (
          ...
          | Ok entries ->
              ...
              let safe_dir = if dir_part = "" || dir_part = "." then "." else dir_part in
              List.iter
                (fun (entry_name, bytes, kind) ->
                  (* I-5: renameByKind takes (relPath, kind) — order matters *)
                  let entry_out_rel = rename_by_kind (path_combine safe_dir entry_name) kind in
                  ...
                  | None ->
                      let why =
                        Printf.sprintf "unsafe entry path blocked (traversal): %s"
                          entry_name
                      in
                      summary := Types.tally (Types.Failed (d.Types.rel_path, why)) !summary;
                      cfg.on_event (Log.Failed (d.Types.abs_path, why))
                  | Some entry_out_abs ->
                      if not cfg.dry_run then write_all_bytes entry_out_abs bytes;
                      summary :=
                        Types.tally
                          (Types.Decrypted
                             (entry_out_rel, Int64.of_int (Bytes.length bytes), Types.MZ))
                          !summary;
                      cfg.on_event (Log.Decrypt (d.Types.abs_path, entry_out_abs, "MZ")))
                entries
          | Error msg ->
              summary := Types.tally (Types.Failed (d.Types.rel_path, msg)) !summary;
              cfg.on_event (Log.Failed (d.Types.abs_path, msg)))
      | Types.XP -> (
          ...
          | Ok (entries, _) -> write_rgssad_manifest cfg d entries Types.XP summary
          | Error e ->
      ...
      | Types.VX -> (
          ...
          | Ok (entries, _) -> write_rgssad_manifest cfg d entries Types.VX summary
          | Error e ->
              let msg = rgssad_err_str e in
              summary := Types.tally (Types.Failed (d.Types.rel_path, msg)) !summary;
              cfg.on_event (Log.Failed (d.Types.abs_path, msg)))
      | Types.VXAce ->
          ...
          (try archive_bytes := Io.read_file d.Types.abs_path
           with e ->
             io_failed := true;
             let msg = Printf.sprintf "I/O during open: %s" (Printexc.to_string e) in
             summary := Types.tally (Types.Failed (d.Types.rel_path, msg)) !summary;
             cfg.on_event (Log.Failed (d.Types.abs_path, msg)));
          if (not !io_failed) && Bytes.length !archive_bytes <> 0 then
            extract_vxace_archive !archive_bytes d.Types.rel_path d.Types.abs_path cfg summary)
    detected;

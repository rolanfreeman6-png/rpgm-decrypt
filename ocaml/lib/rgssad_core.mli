...
type parse_error =
  ...
  | Truncated
      (** Failure modes: [ShortHeader] (< 8 bytes), [BadMagic] (no RGSSAD
          prefix), [BadVersion v] (version byte is [v]), [Truncated] (an entry
          ran past the buffer or the table contained no valid entry). *)
...
(** [parse version sentinel buf] parses the archive [buf] requiring version byte
    [version] and end-of-table rule [sentinel]. Returns [Ok (entries, end_pos)]

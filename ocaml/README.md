...
| Mutation testing | done, verified | 7 directed mutants, 7/7 killed after two coverage-gap fixes. |
...
| Why3 + Z3 proofs | done, verified locally (23/23 Valid) | Cameleer is not in the opam repository, so a hand-written WhyML model of the proof targets (axiomatised helpers **defined**: `read_u32_le`, `u32`, `normalize`), discharged by Z3 (`why3 prove -P z3 -a split_goal_full`), proves all 23 goals: `derive_master_key = u32(seed*9+3)`, `read_u32_le` LE formula, the **Zip-Slip invariant** (normalize never yields `..`), `read_entry` clamped bounds, parser `pos` in `[0,len]`. |

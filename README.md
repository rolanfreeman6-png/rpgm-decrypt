...
checks + 12 QCheck properties. The **pure core** — crypto, VX-Ace key
derivation, the RGSSAD parser, the walker — is 80–100% covered; the I/O
modules (`report.run`, `key_discovery`, `log`) are lower because they do
filesystem/JSON/regex work exercised mainly by end-to-end runs on real games,
not by the in-process suite.

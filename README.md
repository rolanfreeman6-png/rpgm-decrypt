...
|---|---|
| 🧬 **Pure functional core** | Narrow `.mli` per module, 92 behavioural checks + 12 QCheck property tests, mutation-tested (7/7 mutants killed). |
...
Generated by **bisect_ppx** (OCaml's coverage tool) over the 92 behavioural
checks + 12 QCheck properties. The numbers split into two tiers: the
...
- **The pure core** — the actual decryption: XOR math, key derivation, the
  ...
  (92 behavioural checks + 12 property tests feed it thousands of crafted
...
rebuilt and run against the 92 behavioural checks + 12 QCheck properties.
...
```bash
# OCaml: 92 behavioural checks + 12 QCheck property tests
